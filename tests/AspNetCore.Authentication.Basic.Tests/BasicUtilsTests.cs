...
namespace MadEyeMatt.AspNetCore.Authentication.Basic.Tests
{
	...
	public class BasicUtilsTests
	{
		...
		public static void BuildClaimsPrincipal_adds_single_identity_with_claims()
		{
			...
			ClaimsPrincipal claimsPrincipal = BasicUtils.BuildClaimsPrincipal(null, schemeName, null, claims);
		...
		{
			string schemeName = "Test";
			ClaimsPrincipal claimsPrincipal = BasicUtils.BuildClaimsPrincipal(null, schemeName, null);
			Assert.NotNull(claimsPrincipal);
			Assert.Single(claimsPrincipal.Identities);
			Assert.NotNull(claimsPrincipal.Identity);
			Assert.Equal(schemeName, claimsPrincipal.Identity.AuthenticationType);
		...
		{
			ClaimsPrincipal claimsPrincipal = BasicUtils.BuildClaimsPrincipal(null, "Test", null);
			Assert.NotNull(claimsPrincipal);
		}
		...
		{
			ClaimsPrincipal claimsPrincipal = BasicUtils.BuildClaimsPrincipal(null, "Test", "Test");
			Assert.NotNull(claimsPrincipal);
		}
		...
		{
			ArgumentNullException exception = Assert.Throws<ArgumentNullException>(() => BasicUtils.BuildClaimsPrincipal(null, null, null));
		...
		public static void BuildClaimsPrincipal_ownerName_adds_Name_and_NameIdentifier_claims()
		{
			...
			string schemeName = "Test";
			ClaimsPrincipal claimsPrincipal = BasicUtils.BuildClaimsPrincipal(ownerName, schemeName, null);
			Assert.NotNull(claimsPrincipal);
			Assert.NotEmpty(claimsPrincipal.Claims);
		...
		public static void BuildClaimsPrincipal_ownerName_adds_Name_and_NameIdentifier_claims_if_not_already_exists()
		{
			...
			ClaimsPrincipal claimsPrincipal = BasicUtils.BuildClaimsPrincipal(ownerName, schemeName, null, claims);
			Assert.NotNull(claimsPrincipal);
			Assert.NotEmpty(claimsPrincipal.Claims);
